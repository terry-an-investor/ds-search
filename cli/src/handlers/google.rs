...
pub async fn handle(session: String, arg: String) -> CmdResult {
    ...
    Ok(match sub {
        ...
        "search" => {
            if sub_arg.is_empty() {
                return Err("search requires query. Optional: 'search <mode> <query>' (mode: all,images,videos,news,shopping,forums,books,web,ai,short_videos)".into());
            }
        ...
        "mode" => {
            ...
            if sub_arg.is_empty() {
                return Err(
                    "mode requires: ai,images,videos,news,shopping,forums,books,web,short_videos"
                        .into(),
                );
            }
            let query = g.current_query().await;
            if query.is_empty() {
                return Err("no active search. Use 'google search <query>' first".into());
            }
        ...
        "recent" => {
            if sub_arg.is_empty() {
                return Err(
                    "recent requires query (e.g. 'recent Rust lang h' for past hour)".into(),
                );
            }
            let parts: Vec<&str> = sub_arg.rsplitn(2, ' ').collect();
            if parts.len() < 2 {
                return Err("usage: google recent <query> <h|d|w|m|y>".into());
            }
            let period = parts[0];
            let query = parts[1];
            let valid = ["h", "d", "w", "m", "y"];
            if !valid.contains(&period) {
                return Err(format!("invalid period '{}'. Use h/d/w/m/y", period).into());
            }
